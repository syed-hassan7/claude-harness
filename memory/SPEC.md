...
**What's mechanized, and what isn't.** This does not verify a review's *quality* or *substance* — only whether some evidence that review-loop or security-audit ran appears anywhere in the session's transcript before a commit. A session could paste those words without ever meaningfully invoking either skill and this check would read it as clean; that's the same class of limit the drift canary's name-proxy has, stated rather than hidden.

**Mechanism (`memory/hooks/review-gate-check.js`, dual-registered on `PostToolUse` matcher `Bash` and `UserPromptSubmit`, one file branching on whether `tool_name` is present in its stdin):**
- On `PostToolUse:Bash`, reads new transcript lines since the session's cursor (`lib.readTranscriptSince`, the same shared reader `canary-check.js` uses) and regex-matches `review-loop`, `security-audit`, `security-review`, `security-spec`, `red-team-desk`, `coderabbit` anywhere in the raw text — not assistant-text-only, so a `Skill` tool invocation's JSON is caught the same as a typed slash command. A match sets a **sticky, per-session** `reviewSeen` flag; it does not reset after a commit.
...
**Deliberately not re-armed per commit.** One review-loop/security-audit sighting anywhere in a session satisfies every later commit in that session — matches how these skills are actually used (one review pass near the end of a work session, then one or more commits), at the accepted cost of not re-flagging a second large unreviewed round shipped later in the same long session. Revisit only if that proves a real gap in practice, not preemptively.
...
**State:** `<scope>/review-gate/state.json` — `{ "<session_id>": { offset, reviewSeen, pending: {at} | null, lastSeen } }`. Same 30-day idle-prune-with-`EXPIRED`-logging shape as the drift canary's `state.json`, so an unsurfaced miss is never silently dropped from the audit trail.

**Revisit condition:** if the marker regex proves too easy to satisfy incidentally (e.g. a session merely discussing these skills without running them gets read as clean), narrow the signal — e.g. require a `Skill` tool_use block specifically — rather than adding a second heuristic on top.
...
- An early, pre-disk-I/O relevance check (mirroring `memory-architecture.js`'s own pattern) skips the entire `readJSON`/`pruneIdle`/`atomicWrite` round-trip for any `Edit`/`Write`/`Read` call that couldn't possibly change either flag below — this hook's matcher is the broadest in the pack, so this check keeps the common case (editing/reading an unrelated file) cheap.
...
**Stage 1 (ships now): pure `WORKFLOW.md` prose, no new hook, no mechanical trigger.** At a natural task boundary — typically right before/after a commit — append one line to `<scope>/episodic/task-log.md`: `task=... approach=... result=... lesson=...` (template: `memory/templates/task-log.md`). Flat markdown bullet, append-only — same lock-free-by-construction safety as `lessons/index.md` (no read-modify-write, so no lock needed). Never injected by any hook; read on demand only.
