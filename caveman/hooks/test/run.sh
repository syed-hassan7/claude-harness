...
# Windows-style form for embedding inside a `node -e "..."` script's SOURCE
# TEXT (e.g. `require('$HOOKS_WIN/caveman-config')`). Git Bash auto-translates
# POSIX-style argv tokens (`node "$HOOKS/x.js"`) on spawn, but a path
# substituted into the middle of a quoted -e string is not argv -- it reaches
# native node.exe as literal text, which cannot resolve `/c/...` paths. $HOOKS
# stays POSIX-style everywhere else in this file (real argv, or shell-side
# path ops); only content going INTO a -e string needs this form.
...
MODE=$(XDG_CONFIG_HOME="$(win_path "$CFG_HOME")" CAVEMAN_DEFAULT_MODE= node -e "
  process.stdout.write(require('$HOOKS_WIN/caveman-config').getDefaultMode());
" 2> "$CFG_ERR")
