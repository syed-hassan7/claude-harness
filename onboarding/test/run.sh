...
run_verify_json() {
  ...
  #
  # `|| true`: verify.js exits nonzero when any tier is BROKEN or a hard claim
  # (secret-guard/ponytail) is unmet -- a real signal for a real install, but
  # this suite reads the JSON body via expect_status, not the exit code, and a
  # fabricated fixture can legitimately report other tiers BROKEN (e.g. a
  # referenced hook file that was never created on disk) without that being
  # the thing under test. Same reasoning as install.sh's own `|| true` on this
  # exact script. Under `set -e`, a bare `var=$(cmd)` propagates cmd's exit
  # status -- without this the whole suite dies mid-test on the first fixture
  # that happens to trip a hard tier, with no FAIL line printed at all.
  CLAUDE_HARNESS_TARGET="$1" XDG_CONFIG_HOME="$2" APPDATA="$2" \
    node "$1/claude-harness/onboarding/verify.js" --json || true
}
...
run_verify_repo() {
  ...
  # `|| true`: see run_verify_json's comment -- same reason, same risk.
  local target="$1" cfg="$2"
  shift 2
  CLAUDE_HARNESS_TARGET="$target" XDG_CONFIG_HOME="$cfg" APPDATA="$cfg" \
    node "$REPO_DIR/onboarding/verify.js" "$@" || true
}
...
# Real settings.json shape -- verify.js's wiring tier expects each event to be
# an ARRAY of { hooks: [{ command }] } entries (what install.sh actually
# generates), not a bare filename string. A shorthand fixture here crashed the
# (unrelated) wiring tier before Test 6 could even reach the 4 tiers it
# checks -- one hook per event this pack's install actually wires is enough
# to make the fixture representative without pulling in every hook.
node -e '
const fs = require("fs"), pack = process.argv[1];
const n = (rel) => ({ hooks: [{ type: "command", command: `node "${pack}/${rel}"`, timeout: 5 }] });
fs.writeFileSync(process.argv[2], JSON.stringify({
  enabledPlugins: { "ponytail@ponytail": true },
  hooks: {
    SessionStart: [n("caveman/caveman-activate.js"), n("memory/hooks/memory-init.js")],
    SessionEnd: [n("memory/hooks/memory-flush.js")],
  },
}, null, 1));
' "$T6/claude-harness" "$T6/settings.json"
